...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 4 and sys.argv[1] == "-t":
        if len(sys.argv) >= 5:
            _alice_offset = int(sys.argv[4], 16)
        outAliceFile = sys.argv[3]
        alicefile = sys.argv[2]
        notranslate = 1
    elif len(sys.argv) >= 3 and sys.argv[1] != "-t":
        if len(sys.argv) >= 4:
            _alice_offset = int(sys.argv[3], 16)
        outAliceFile = sys.argv[2]
        alicefile = sys.argv[1]
    else:
        print("usage: unalice.py [-t] <ALICE> <OUTPUT> [ALICE_OFFSET]")
        print("       -t disable bl/blx addr translation (required for some images)")
        sys.exit()

    _f_ptr = open(alicefile, "rb")
    if _alice_offset == -1:
        _alice_offset = bytearray(_f_ptr.read()).index(b"ALICE_")
        
    _f_ptr.seek(_alice_offset)
    f = io.BytesIO(_f_ptr.read())
    magic = f.read(7)
    if magic == b'ALICE_1':
        alice_version = 1
    ...
    fout = open(f"{os.path.splitext(outAliceFile)[0]}_trans{os.path.splitext(outAliceFile)[1]}", "wb")
    alicebin = bytearray()
    ...
    falicebin = open(outAliceFile, "wb")
