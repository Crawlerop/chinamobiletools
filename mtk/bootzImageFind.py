...
if __name__ == "__main__":    
    ...
    for f in gfs:
        ...
        if fileinfo.info_type == 0x108:
            ...
            zImage = viva.zPartition.parse(filedata[vivaHead.boot_zimage:])           
            
            fOut = open(sys.argv[3], "wb")

            for part in zImage.partitions:                
                if part.compression_type == 3: # 3 = LZMA with Train from ALICE
                    fOut.write(mtklzma.transformProcess(mtklzma.decompress(part.data, zImageDict)))
                else: # 0 = LZMA, 1 = LZMA with callback, 2 = Same as 0
                    fOut.write(mtklzma.transformProcess(mtklzma.decompress(part.data)))
