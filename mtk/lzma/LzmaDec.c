  ...
  do
  {
    ...
    
    {
      ...
      if (state >= kNumStates)
      {
        ...
        if (distance >= p->trainSize+(checkDicSize == 0 ? processedPos: checkDicSize))
        {
...
SRes LzmaDecodeTrained(Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
    const Byte *propData, unsigned propSize, ELzmaFinishMode finishMode,
    ELzmaStatus *status, ISzAllocPtr alloc, Byte *train, SizeT *trainLen)
{
  CLzmaDec p;
  SRes res;
  SizeT outSize = *destLen, inSize = *srcLen, trainSize = *trainLen;
  *destLen = *srcLen = 0;
  *status = LZMA_STATUS_NOT_SPECIFIED;
  if (inSize < RC_INIT_SIZE)
    return SZ_ERROR_INPUT_EOF;
  LzmaDec_CONSTRUCT(&p)
  RINOK(LzmaDec_AllocateProbs(&p, propData, propSize, alloc))
  p.dic = malloc(outSize+trainSize);
  if (!p.dic) {
    return SZ_ERROR_MEM;
  }
  memcpy(p.dic, train, trainSize);
  p.dicBufSize = outSize+trainSize;
  LzmaDec_Init(&p);
  p.dicPos = trainSize;
  p.trainSize = trainSize;
  *srcLen = inSize;
  res = LzmaDec_DecodeToDic(&p, outSize+trainSize, src, srcLen, finishMode, status);
  *destLen = p.dicPos-trainSize;
  if (res == SZ_OK && *status == LZMA_STATUS_NEEDS_MORE_INPUT)
    res = SZ_ERROR_INPUT_EOF;
  if (res == SZ_OK)
    memcpy(dest, p.dic+trainSize, *destLen);
  free(p.dic);
  LzmaDec_FreeProbs(&p, alloc);
  return res;
}
...
SRes lzma_uncompress(Byte *dest, SizeT destLen, const Byte *src, SizeT srcLen) {
  SizeT srcLength = srcLen - 13;
  ELzmaStatus lzmaStatus;

  return LzmaDecode(dest, &destLen, src + 13, &srcLength,
    src, 5, 0, &lzmaStatus, &_allocFuncs);
};

SRes lzma_uncompress_train(Byte *dest, SizeT destLen, const Byte *src, SizeT srcLen, Byte *dict, SizeT dictLen) {
  SizeT srcLength = srcLen - 13;
  ELzmaStatus lzmaStatus;

  return LzmaDecodeTrained(dest, &destLen, src + 13, &srcLength,
    src, 5, 0, &lzmaStatus, &_allocFuncs, dict, &dictLen);
};
...
void transformProcess(Byte *data, SizeT data_size) {
  byte bVar1;
  uint uVar2;
  uint uVar3;
  uint uVar4;
  ...
  uVar2 = 0;
  while (uVar2 <= data_size - 4U) {
    uVar3 = uVar2;
    if (data[uVar2 + 1] >> 3 == 0x1e) {
      bVar1 = data[uVar2 + 3];
      if ((bVar1 >> 3 == 0x1f) || (bVar1 >> 3 == 0x1d)) {
        uVar3 = (((uint)data[uVar2 + 2] | (uint)data[uVar2 + 1] << 0x13 |
                 (uint)data[uVar2] << 0xb | ((uint)bVar1 << 0x1d) >> 0x15) * 2 - uVar2) - 4;
        uVar4 = uVar3 >> 1;
        data[uVar2 + 1] = (byte)((uVar4 << 10) >> 0x1d) - 0x10;
        data[uVar2] = (uchar)(uVar3 >> 0xc);
        data[uVar2 + 3] = bVar1 & 0xf8 | (byte)((uVar4 << 0x15) >> 0x1d);
        uVar3 = uVar2 + 2;
        data[uVar2 + 2] = (uchar)uVar4;
      }
    }
    uVar2 = uVar3 + 2;
  }
